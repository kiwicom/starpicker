                ...
                try:
                    for review, inner_kwargs in self.parse(response, **kwargs):
                        ...
                        try:
                            ...
                            LOG.exception(
                                "error while parsing a %s review, review was %s",
                                self,
                                review,
                            )
                except:
                    LOG.exception(
                        "error while parsing for %s reviews, response was %s",
                        self,
                        response.text,
                    )
        except:
...
class TrustpilotReviewCollector(BaseCollector):
    ...
    url = "https://api.trustpilot.com/v1/business-units/{config.TRUSTPILOT_BUSINESS_ID}/reviews".format(
        config=config
    )
...
class FacebookRatingCollector(BaseCollector):
    ...
    url = "https://graph.facebook.com/v3.2/{config.FACEBOOK_PAGE_ID}/ratings".format(
        config=config
    )
    params = {
        "fields": "open_graph_story,review_text,rating",
...
class FacebookCommentCollector(BaseCollector):
    ...
    url = "https://graph.facebook.com/v3.2/{config.FACEBOOK_PAGE_ID}/feed".format(
        config=config
    )
    params = {
        "fields": "comments{id,message,from,permalink_url}",
        "access_token": config.FACEBOOK_ACCESS_TOKEN,
    }
...
class TweetCollector(BaseCollector):
    ...
    auth_code = b64encode(
        "{0}:{1}".format(config.TWITTER_APP_KEY, config.TWITTER_APP_SECRET).encode()
    )
    ...
    def fetch(self):
        ...
        for sentiment in (":(", "", ":)"):
            yield requests.get(
                self.url,
                params={
                    "q": "{0} {1}".format(config.TWITTER_SEARCH_KEYWORD, sentiment),
                    "result_type": "recent",
                },
